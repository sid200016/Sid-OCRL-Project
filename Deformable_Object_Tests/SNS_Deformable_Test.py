    ...
    while (True):
        if statusDict["Hardware Initialized"] == True:
            ...
            if s.upper() == 'Y':
                ...
                fincal = await aioconsole.ainput("Use joystick to move and position grasper. Enter 'F' when finished with calibration.")
                match fincal.upper():
                    case "F":
                        statusDict["Finished Calibration"] = True
                        print("Robot calibration values:\n x:%f, y:%f, z:%f, closure:%f, closure pressure:%f, jaw pressure 1: %f, , jaw pressure 2: %f, jaw pressure 3: %f"%tuple([v for k,v in robotCalibration_vals.items()]))


                        useXYZcalibration = await aioconsole.ainput(
                            "Do you want to use the XYZ position from the calibration for the target position? Enter 'Y' for yes, 'N' to use default position, 'T' to adjust the calibration xyz value, or 'X' to enter a new value.")
                        ...
                        if statusDict["sendCalibrationXYZ"] == True:
                            sio.emit("object-location",{"x_m":robotCalibration_Vals["x_mm"]/1000,"y_m":robotCalibration_Vals["y_mm"]/1000,"z_m":robotCalibration_Vals["z_mm"]/1000})
                        ...
                        useGrasperCalibration = await aioconsole.ainput(
                            "Do you want to use the grasper maximum change in radial position from the calibration for the target position? Enter 'Y' for yes, 'N' to use default position, 'T' to adjust the calibration grasper value, or 'X' to enter a new value.")
                        ...
                        if statusDict["sendCalibrationGrasper"] == True:
                            sio.emit("grasper-max",robotCalibration_Vals["grasper_width_mm"])
