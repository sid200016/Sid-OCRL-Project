...
tau = torch.Tensor(0.3*np.ones(COMMAND_LAYER_SIZE))
...
tau = torch.Tensor(0.3*np.ones(INTER_LAYER_1_SIZE))
inter_layer_1 = SNS_layer(layer_input_size=INTER_LAYER_1_INPUT_SIZE,
                          layer_size=INTER_LAYER_1_SIZE, sparsity_mask=sparsity_mask, R=R,tau=tau)
...
modulation_sparsity_mask = np.zeros([MOTOR_LAYER_MODULATION_INPUT_SIZE, MOTOR_LAYER_SIZE], dtype=np.int32)
modulation_sparsity_mask[12, 3] = 1
...
sparsity_mask_2 = np.zeros(
    [MOTOR_LAYER_MODULATION_INPUT_SIZE, MOTOR_LAYER_SIZE], dtype=np.int32)
for i in range(2):  # object_x = x joint command, object_y = y joint command/target_x = x joint command, target_y = y joint command
    sparsity_mask_2[[4 * i, 4 * i + 2], i] = 1
    sparsity_mask_2[[4 * i + 1, 4 * i + 3], i] = -1
# object_z = z joint command/target_z = z joint command
sparsity_mask_2[8:10, 2] = 1
sparsity_mask_2[10, 2] = -1  # lift the gripper up = decrease z joint command
# open the gripper = negative left claw joint command + positive right claw joint command
sparsity_mask_2[11, 3] = 1

theta_max_out = torch.Tensor(THETA_MAX)
theta_max_out_modulation = torch.Tensor(THETA_MAX)
theta_max_out_modulation[-1] *= 0.2
theta_min_out = torch.Tensor(THETA_MIN)
tau = torch.Tensor([0.1, 0.3, 0.1, 0.1])
...
modulation_motor_layer = modulation_SNS_layer(layer_input_size=MOTOR_LAYER_MODULATION_INPUT_SIZE, layer_size=MOTOR_LAYER_SIZE,
                        sparsity_mask=sparsity_mask_2, modulation_sparsity_mask=modulation_sparsity_mask, tau=tau, theta_min_out=theta_min_out, theta_max_out=theta_max_out_modulation, R=R)

motor_layer._params["sensory_erev"].data[10, 2] = -R / 10
modulation_motor_layer._params["sensory_erev"].data[10, 2] = -R / 10
...
modulation_controller = SNS_Modulation_Control(inter_layer_1, inter_layer_2, modulation_motor_layer)
perceptor.eval()
controller.eval()
modulation_controller.eval()