...
class SNSCell_modulation(nn.Module):
    def __init__(
        self,
        wiring,
        in_features=None,
        ...
        super(SNSCell_modulation, self).__init__()
    ...
    def buffer_size(self):
        return np.max(np.round(self._params["t_d"].numpy()/self._elapsed_time*self._ode_unfolds,decimals=0).astype(int))
    ...
    def _allocate_parameters(self):
        ...
        self._params["tau_io"] = self.add_weight(
            name="tau_io", init_value=self._get_init_value((self.state_size,), "tau_io")
        )
        self._params["t_d"] = self.add_weight(
            name="t_d", init_value=self._get_init_value((self.state_size,), "t_d")
        )
        self._params["beta"] = self.add_weight(
            name="beta", init_value=self._get_init_value((self.state_size,), "beta")
        )
        self._params["k_io"] = self.add_weight(
            name="k_io", init_value=self._get_init_value((self.state_size,), "k_io")
        )
        ...
        self._params["modulation_sparsity_mask"] = torch.Tensor(
            np.abs(self._wiring.modulation_adjacency_matrix)
        )
    ...
    def _ode_solver(self, inputs, state, a, buffer, elapsed_time):
        v_pre = state
        ...
        sensory_modulation_activation = sensory_matrix*self._params["modulation_sparsity_mask"]
        ...
        modulation_activation = torch.sum(sensory_modulation_activation, dim=1)
        ...
        for t in range(self._ode_unfolds):
            ...
            u = buffer[np.arange(buffer.shape[0]),self.buffer_size - 1]
            a = (u + self._params["tau_io"] / delta * a) / (self._params["beta"] + (1 - self._params["beta"]) * u + self._params["tau_io"] / delta)
            buffer[:,1:] = buffer[:,:-1].clone()
            buffer[:,0] = modulation_activation

        return v_pre, a, buffer
    ...
    def _map_outputs(self, state, a):
        output = state
        if self.motor_size < self.state_size:
            #output = output[:, 0 : self.motor_size]  # slice
            output = output[:, -self.motor_size:]
        if self._output_mapping in ["affine", "linear"]:
            output = output * self._params["output_w"] * (1 + self._params["k_io"].data * a)
        if self._output_mapping == "affine":
    ...
    def forward(self, inputs, states, a, buffer):
        # Regularly sampled mode (elapsed time = 1 second)
        #elapsed_time = 0.1

        inputs = self._map_inputs(inputs)

        next_state, next_a, next_buffer  = self._ode_solver(inputs, states, a, buffer, self._elapsed_time)

        outputs = self._map_outputs(next_state, next_a)

        return outputs, next_state, next_a, next_buffer