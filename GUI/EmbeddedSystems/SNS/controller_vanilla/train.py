...
from .SNS_layer import SNS_layer, SENSORY_LAYER_1_INPUT_SIZE, SENSORY_LAYER_1_SIZE, SENSORY_LAYER_2_INPUT_SIZE, \
    SENSORY_LAYER_2_SIZE, THETA_MAX, THETA_MIN, F_MAX, F_MIN, sensory_layer_1, R
...
def training_dataloader(in_features=SENSORY_LAYER_1_INPUT_SIZE + 3, out_features=SENSORY_LAYER_2_SIZE, N=SEQUENCE_LENGTH, batch_size=BATCH_SIZE, training_examples=TRAINING_EXAMPLES, ratio=RATIO, std=STD, mu=MU, theta_max=THETA_MAX, theta_min=THETA_MIN, f_max=F_MAX, f_min=F_MIN):
    ...
    for i in range(training_examples):
        ...
        pon = np.random.rand(1)
        if pon < 1/3:
            mu_selected = mu[1]
            force_feedback = mu_selected + np.random.normal(0, 0.01/2, 3) * np.abs(force_max - force_min) * 2
        elif pon > 2/3:
            mu_selected = mu[0]
            force_feedback = (mu_selected + np.random.normal(0, 0.01/2, 3) * np.abs(force_max - force_min) * 2) / 3
        else:
            mu_selected = np.random.choice(mu)
            force_feedback = force_min + np.random.rand(3) * (force_max - force_min)
        ...
        grasped_1 = (force_feedback[0] > MU[-1]) * 1.0
        grasped_2 = (force_feedback[1] > MU[-1]) * 1.0
        grasped_3 = (force_feedback[2] > MU[-1]) * 1.0
        released = (np.sum(force_feedback) > MU[0]) * 1.0
        sensory_layer_2_out = np.array([move_to_pre_grasp, move_to_grasp, move_to_pre_target, move_to_target, grasped_1, grasped_2, grasped_3, released])
...
#torch.save(SNS_sequence._sensory_layer_2.state_dict(), "sensory_layer_2_param")
#torch.save(SNS_sequence._output_mu, "output_mu_param")
#torch.save(SNS_sequence._output_sigma, "output_sigma_param")